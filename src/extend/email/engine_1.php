...
class engine_1 extends config
{
    var $smtp_port; var $time_out; var $host_name; var $log_file;
    var $relay_host; var $debugs; var $auth; var $user; var $pass; var $sock;
    public static function send($to, $subject, $content)
    {
        ...
        if(config::$secure == "ssl") {
            ...
            // 处理昵称
            $nickname = batch_str_replace(config::$nickname, array("[:sitename:]"), array(config::$sitename));
            $object = new self;
            // 这里面的一个 true 是表示使用身份验证,否则不使用身份验证.
            $object->smtp(config::$server, (int)config::$port, true, config::$username, config::$password);
            // 检测是否开启 Debug 模式
            $object->debugs = (strtolower(config::$debug) == "true") ? true : false;
            $result = $object->sendmail($to, config::$username, $nickname, $subject, $content, "HTML");
    ...
    public function sendmail($to, $from, $name, $subject = "", $body = "", $mailtype, $cc = "", $bcc = "", $additional_headers = "")
    {
    ...
    public function smtp_sockopen_relay()
    {
        ...
        if (!($this->sock && $this->smtp_ok())) {
            ...
            $errstr = mb_convert_encoding($errstr, 'utf-8', 'gbk');
